...
(excersize = [
  ...
  (exports.UPDATE_PLAN = async (req, res) => {
    ...
    try {
      let currentUserPlan = await weekExcersize
        .findOne({
          user: req.user._id,
          _id: week_excersize_id,
          week: week,
          day: day,
        })
        .sort({ createdAt: 1 });
      // let mainPlan = await weekExcersize.findOne({user:null,week:week,day:day}).sort({createdAt:1})
      let excersizes = await excersizeModel
        .find({ session_no: session_no })
        .select({ _id: 1 });
...
exports.GET_ALL_LEFT_OVER_EXCERSIZES = async (req, res) => {
  ...
  try {
    const weekexcersize = await weekExcersize
      .find({
        user: user_id,
    ...
    for (let i = 0; i < weekexcersize.length; i++) {
      for (let j = 0; j < weekexcersize[i].excersize.length; j++) {
        if (weekexcersize[i].excersize[j].excersize != null) {
          weekexcersize[i].excersize[
            j
          ].excersize.type_image = `${url}${weekexcersize[i].excersize[j].excersize.type_image}`;
          weekexcersize[i].excersize[
            j
          ].excersize.image = `${url}${weekexcersize[i].excersize[j].excersize.image}`;
        }
      }
    }
...
exports.ADD_WEEK_EXCERSIZE = async (req, res, next) => {
  try {
    ...
    let weekexcersize = await weekExcersize.findOne({
      week: req.body.week,
      day: req.body.day,
      user: req.body.user,
    });
...
exports.GET_WEEK_EXCERSIZES = async (req, res) => {
  ...
  try {
    let weekexcersize = await weekExcersize
      .find()
      .populate("excersize")
      .limit(per_page)
      .skip(offset)
    ...
    if (!weekexcersize.length) {
      return res.status(400).json({ message: "No week Excersize Exist" });
    }
...
exports.GET_WEEK_EXCERISZE_DETAIL_BY_ID = async (req, res) => {
  ...
  try {
    ...
    if (!weekexcersize)
      return res
        .status(400)
        .json({ message: "Week Excersize Detail not found" });
  ...
  try {
    const weekexcersize = await weekExcersize
      .findOne({
        user: user_id,
        week: currentweek,
        day: weekday[currentday].toLowerCase(),
      })
      .populate("excersize.excersize");
    const url = baseUrl(req);

    if (!weekexcersize)
      return res.status(400).json({ message: "Week Excersize  not found" });
    for (let i = 0; i < weekexcersize.excersize.length; i++) {
      if (weekexcersize.excersize[i].excersize != null) {
        weekexcersize.excersize[
          i
        ].excersize.type_image = `${url}${weekexcersize.excersize[i].excersize.type_image}`;
        weekexcersize.excersize[
          i
        ].excersize.image = `${url}${weekexcersize.excersize[i].excersize.image}`;
      }
    }
...
exports.GET_ALL_WEEK_EXCERCISES_OF_CURRENT_USER = async (req, res) => {
  ...
  try {
    const weekexcersizes = await weekExcersize
      .find({
        // ...weekfilter,
        // week: { $lte: currentweek },
        user: user_id,
      ...
      for (let j = 0; j < weekexcersizes[i].excersize.length; j++) {
        if (weekexcersizes[i].excersize[j].excersize != null) {
          if (j == 0) {
            ...
            weekexcersizes[i].session_no =
              weekexcersizes[i].excersize[0].excersize.session_no;
          }
          weekexcersizes[i].excersize[
            j
          ].excersize.type_image = `${url}${weekexcersizes[i].excersize[j].excersize.type_image}`;
          weekexcersizes[i].excersize[
            j
          ].excersize.image = `${url}${weekexcersizes[i].excersize[j].excersize.image}`;
        }
      }
    }
...
exports.UPDATE_USER_EXCERSIZE_STATUS = async (req, res) => {
  ...
  try {
    ...
    let index = weekexcersize.excersize.findIndex(
      (item) => item._id == excersize_id
    );
...
exports.GET_BASIC_PLAN = async (req, res) => {
  try {
    let weekexcersizes = await weekExcersize
      .find({ user: null, is_off: false })
      .populate("excersize.excersize")
      .sort({ createdAt: 1 });
    const url = baseUrl(req);
    // console.log(weekexcersizes)
